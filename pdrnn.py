...
def triangular_pmf(x,
    ...
    L = (tf.ceil(mu-sigma)+tf.floor(mu)-2*mu) / (2*sigma)
    R = (2*mu-tf.ceil(mu)-tf.floor(mu+sigma)) / (2*sigma)
    
    h_inv = (tf.floor(mu)-tf.floor(mu-sigma))*(L+1) + (tf.floor(mu+sigma)-tf.floor(mu))*(R+1)
